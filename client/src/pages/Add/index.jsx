...
export default function Add() {
  ...
  const analyzeClothes = async () => {
    if (!imageUrl) {
      Toast.show({ icon: 'fail', content: '请先上传衣物图片', duration: 1200 })
      return
    }
    ...
    try {
      ...
      if (res.meta?.elapsed) {
        console.log(`[分析耗时] 模式: ${analysisMode}, 服务端耗时: ${res.meta.elapsed}`)
      }
  ...
  const handleUploadCloth = async () => {
    ...
    try {
      ...
      Toast.show({ icon: 'success', content: '上传成功', duration: 1200 })
      markMatchStale()
      ...
      if (axios.isCancel(error) || error.name === 'AbortError' || error.code === 'ERR_CANCELED') {
        ...
        const message = getErrorMessage(error, '上传失败，请重试')
    ...
    <div className={styles.add}>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.submit}>
          <Button color="primary" block onClick={handleUploadCloth} loading={uploading} disabled={uploading}>
            确认添加到衣橱
          </Button>
          {uploadController ? (
