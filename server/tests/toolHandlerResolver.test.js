...
test('handler resolver can resolve catalog handler strings to executable functions', () => {
  ...
  const exportClosetDataTool = getToolByName('export_closet_data')
  const importClosetDataTool = getToolByName('import_closet_data')
  const updateClothImageTool = getToolByName('update_cloth_image')
  const updateOutfitLogTool = getToolByName('update_outfit_log')
  const updateUserNameTool = getToolByName('update_user_name')
  const uploadUserAvatarTool = getToolByName('upload_user_avatar')
  const uploadCharacterModelTool = getToolByName('upload_character_model')
  const deleteCharacterModelTool = getToolByName('delete_character_model')
  ...
  const exportClosetDataHandler = resolveToolHandler(exportClosetDataTool)
  const importClosetDataHandler = resolveToolHandler(importClosetDataTool)
  const updateClothImageHandler = resolveToolHandler(updateClothImageTool)
  const updateOutfitLogHandler = resolveToolHandler(updateOutfitLogTool)
  const updateUserNameHandler = resolveToolHandler(updateUserNameTool)
  const uploadUserAvatarHandler = resolveToolHandler(uploadUserAvatarTool)
  const uploadCharacterModelHandler = resolveToolHandler(uploadCharacterModelTool)
  const deleteCharacterModelHandler = resolveToolHandler(deleteCharacterModelTool)
  ...
  assert.equal(typeof exportClosetDataHandler, 'function')
  assert.equal(typeof importClosetDataHandler, 'function')
  assert.equal(typeof updateClothImageHandler, 'function')
  assert.equal(typeof updateOutfitLogHandler, 'function')
  assert.equal(typeof updateUserNameHandler, 'function')
  assert.equal(typeof uploadUserAvatarHandler, 'function')
  assert.equal(typeof uploadCharacterModelHandler, 'function')
  assert.equal(typeof deleteCharacterModelHandler, 'function')
