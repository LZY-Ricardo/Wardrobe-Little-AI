...
const sfBaseUrl = (process.env.SILICONFLOW_BASE_URL || 'https://api.siliconflow.cn/v1').replace(/\/$/, '')
const sfVisionModel = process.env.SILICONFLOW_VISION_MODEL || 'Qwen/Qwen3-VL-8B-Instruct'
const sfTimeoutMs = Number(process.env.SILICONFLOW_TIMEOUT_MS) || 30000
...
const ensureConfig = () => {
  if (!sfApiKey || sfApiKey === 'your_siliconflow_api_key') {
...
const analyzeWithVision = async (imageBuffer, mimeType) => {
  ...
  console.log(`[Vision] 请求模型: ${sfVisionModel}, base64 大小: ${(base64.length / 1024).toFixed(1)}KB`)

  let response
  try {
    response = await axios.post(`${sfBaseUrl}/chat/completions`, {
      model: sfVisionModel,
      messages: [{
        role: 'user',
        content: [
    ...
    console.error(`[Vision] API 错误 ${status}:`, JSON.stringify(data))
    const msg = data?.error?.message || data?.message || err.message
    throw new Error(`SiliconFlow API 错误(${status}): ${msg}`)
  }
...
const analyzeClothesVision = async (ctx) => {
  try {
    ...
    if (sfBreaker.isOpen()) {
      ctx.status = 503
      ctx.body = { code: 0, msg: '视觉分析服务繁忙，请稍后重试' }
      return
    }
    ...
    const rawResult = await sfBreaker.exec(() => analyzeWithVision(imageFile.buffer, imageFile.mimetype))
    ...
    ctx.body = { code: 1, data: JSON.stringify(analysisResult), meta: { elapsed: `${elapsed}s`, model: sfVisionModel } }
  } catch (error) {
...
module.exports = { analyzeClothesVision }
