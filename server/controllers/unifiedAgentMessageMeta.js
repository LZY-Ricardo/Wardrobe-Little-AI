...
const KNOWN_CONFIRMATION_STATUSES = new Set(['pending', 'confirmed', 'cancelled'])
...
const parseMessageMeta = (raw) => {
  ...
  try {
    ...
    return normalizeMessageMeta(parsed)
  } catch {
    return null
  }
}
...
const normalizeMessageType = (messageType = '') => {
  const normalized = clampString(messageType, 40)
  return KNOWN_MESSAGE_TYPES.has(normalized) ? normalized : 'chat'
}

const normalizeConfirmationStatus = (confirmationStatus = '') => {
  const normalized = clampString(confirmationStatus, 40)
  if (!normalized) return ''
  return KNOWN_CONFIRMATION_STATUSES.has(normalized) ? normalized : ''
}
...
const hydrateMessage = (message) => {
  ...
  return {
    ...
    confirmation_status: normalizeConfirmationStatus(message.confirmation_status),
    meta,
