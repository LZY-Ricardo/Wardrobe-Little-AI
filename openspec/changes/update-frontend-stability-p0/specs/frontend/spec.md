...
### Requirement: 受保护路由鉴权
系统 MUST 对受保护路由进行登录状态校验，保障未登录用户无法访问业务页面。
...
前端请求 MUST 统一超时与错误提示，刷新 token 需单通道，认证失效时清理状态并引导重新登录。
...
推荐页面 MUST 调用 `/scene/generateSceneSuits` 渲染结果，并在异常时优雅降级。
...
### Requirement: 上传与分析降级
上传与分析链路 MUST 提供错误提示与手动兜底，避免阻塞录入。
