...
## 当前落地进度（已按 P0 → P1/P2 迭代）

- ✅ P0：穿搭档案编辑弹窗完成“块状分组 + Label Top + Grid”，底部通栏主按钮保存
- ✅ P0：尺码使用 `react-vant` `Picker`，偏好使用 `react-vant` `Selector`（多选）
- ✅ P0：人物模特入口收敛为“上传 + 管理”，个人页不默认展示大图（避免占空间）
- ✅ P0：补齐暗色模式关键样式覆盖（使用 `client/src/index.less` 的主题变量）

> 注：本文中的“现状问题/行号索引”更多用于分析与规划，实际实现以代码为准。
